...
class Cella:
    def __init__(
        self, indices, env_space_temps, env_space_org_density, env_nutrients
    ) -> None:
        ...
        self.cell_organisms = []
        self.cell_nutrients = {
            key: round(random.uniform(low, high), 3)
            for key, (low, high) in env_nutrients.items()
        }
...
class Environment:
    def __init__(
        ...
        self.organisms = sorted(
            [
                Organism.create(
                    np.random.randint(0, env_grid_size[0], size=3), org_type
                )
        ...
        self.env_time = 0
        self.env_space_temps = np.random.uniform(
            temp_init_range[0], temp_init_range[1], env_grid_size
        )
        self.env_space_org_density = np.zeros(
            shape=(self.x_max, self.y_max, self.z_max)
        )

        self.env_space = {
            f"{x}-{y}-{z}": Cella(
                (x, y, z),
                self.env_space_temps,
                self.env_space_org_density,
                env_nutrients,
            )
        ...
        for org in self.organisms:
            self.env_space[f"{org.x}-{org.y}-{org.z}"].cell_organisms.append(org)
            self.env_space_org_density[org.x, org.y, org.z] += 1

    def step(self):
        # Environment change temps
        self.env_space_temps = np.where(
            self.env_space_org_density == 0,
            self.env_space_temps + self.temp_decr,
            self.env_space_temps,
        )
        self.env_space_temps = np.where(
            self.env_space_org_density > 0,
            self.env_space_temps + self.temp_decr * 0.1,
            self.env_space_temps,
        )
    ...
    def update_org_position(self, org, dx, dy, dz):
        ...
        self.env_space[f"{old_pos[0]}-{old_pos[1]}-{old_pos[2]}"].cell_organisms.remove(
            org
        )
        self.env_space[f"{new_pos[0]}-{new_pos[1]}-{new_pos[2]}"].cell_organisms.append(
            org
        )
